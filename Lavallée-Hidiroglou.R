#==============================================================================
#===      ESTRATIFICACIÓN MEDIANTE EL MÉTODO DE LAVALLÉE-HIDIROGLOU         ===
#==============================================================================
...
M <- strata.LH(x = x,
               CV = 0.05, #coeficiente de variación deseado
               Ls = 3, #número total de estratos
               alloc = list(q1 = 0.5, q2 = 0, q3 = 0.5), #afijación de Neyman
               takeall = 1, #incluir un estrato take-all
               algo = "Sethi")  #emplea el algoritmo de Sethi (1963)

# (4) Resultados:
bounds <- M$bh; bounds #límites óptimos de los estratos
M$n; #tamaño de muestra
...
#     (a) Algoritmo de Sethi con afijación proporcional:      
M <- strata.LH(x = x,
               CV = 0.05, #coeficiente de variación deseado
               Ls = 3, #número total de estratos
...
Ne <- M$Nh; Ne #tamaños poblacionales de cada estrato
sizes <- M$nh; sizes #tamaños de muestra en cada estrato
U1 <- x[x<=bounds[1]]; length(U1)
U2 <- x[x>bounds[1] & x<=bounds[2]]; length(U2)
U3 <- x[x>bounds[2]]; length(U3)
set.seed(2022)
idx1 <- sample(seq_along(U1), size = sizes[1], replace = FALSE); idx2 <- sample(seq_along(U2), size = sizes[2], replace = FALSE)
yU1 <- y[idx1]
yU2 <- y[idx2+Ne[1]]
...
#     (b) Algoritmo de Sethi con afijación de potencia (p=1):      
M <- strata.LH(x = x,
               CV = 0.05, #coeficiente de variación deseado
               Ls = 3, #número total de estratos
...
#     (c) Algoritmo de Kozak con afijación proporcional:      
M <- strata.LH(x = x,
               CV = 0.05, #coeficiente de variación deseado
               Ls = 3, #número total de estratos
               alloc = list(q1 = 0.5, q2 = 0, q3 = 0), #afijación proporcional
               takeall = 1, #incluir un estrato take-all
               algo = "Kozak")  #emplea el algoritmo de Kozak (2004)
bounds <- M$bh; bounds #límites óptimos de los estratos
M$n; #tamaño de muestra
Ne <- M$Nh; Ne #tamaños poblacionales de cada estrato
sizes <- M$nh; sizes #tamaños de muestra en cada estrato
...
#     (d) Algoritmo de Kozak con afijación de potencia (p=1):      
M <- strata.LH(x = x,
               CV = 0.05, #coeficiente de variación deseado
               Ls = 3, #número total de estratos
               alloc = list(q1 = 0.5, q2 = 0.5, q3 = 0), #afijación de potencia
...
#     (e) Algoritmo de Kozak con afijación de Neyman (default):      
M <- strata.LH(x = x,
               CV = 0.05, #coeficiente de variación deseado
               Ls = 3, #número total de estratos
               alloc = list(q1 = 0.5, q2 = 0, q3 = 0.5), #afijación de Neyman
